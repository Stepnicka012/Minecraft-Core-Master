...
class ForgeDownloader extends EventEmitter {
  ...
  constructor(options: DownloaderOptions = {}) {
    ...
    this.options = {
      version: '', // Valor por defecto vacío
      javaPath: 'java',
      root: '.minecraft',
      concurrency: 1,
      maxRetries: 3,
  ...
  // Método principal para iniciar la descarga e instalación
  ...
  private async runInstaller(installerPath: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Capturar logs si está en verbose
      if (this.options.verbose) {
  ...
  // Tipado de eventos
  on<K extends keyof DownloaderEvents>(
    event: K, 
  ...
  emit<K extends keyof DownloaderEvents>(
    event: K, 

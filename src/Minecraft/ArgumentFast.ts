...
async function processLibraries(
    ...
    if (isLegacy) {
      ...
      if (libPath && !processedPaths.has(libPath)) {
        try {
          ...
          libraryCount++;
          if (options?.enableDebug) {
            console.log(`Añadido a classpath (legacy): ${libPath}`);
            if (lib.name.includes('lwjgl')) console.log(`[ LWJGL Nativo agregado ]: ${lib.name} -> ${libPath}`);
          }
        } catch (error) {
          if (options?.enableDebug) console.warn(`Libreria no encontrada (legacy): ${libPath}`);
        }
      }
      continue;
    }
        ...
        if (nativeArtifact) {
          ...
          try {
            ...
            try {
              ...
              if (options?.enableDebug) console.warn(`Error copiando librería nativa: ${fileName}`, copyError);
            }
            libraryCount++;
  ...
  if (options?.enableDebug) {
    try {
      ...
      if (missingNatives.length > 0) {
        console.log(`[ ADVERTENCIA ]: Archivos nativos críticos faltantes para ${osCache} (${archCache}):`);
        console.log(` Esperados: ${criticalNatives.filter(n => !n.includes('64') || archCache === 'x64').join(', ')}`);
        ...
        if (similarFiles.length > 0) console.log(`Archivos similares encontrados: ${similarFiles.join(', ')}`);
      } else console.log(`Sistema ${osCache} (${archCache}): Nativos correctamente extraídos`);
    } catch (error) {
  ...
  if (options?.enableDebug) {
    ...
    classpath.forEach((path, index) => console.log(`  [${index + 1}] ${path}`));
...
export async function ArgumentsBuilder(options: LauncherOptions): Promise<LaunchResult> {
  ...
  try {
    ...
    emitter.emit("launch-complete", { pid: child.pid, totalTime: stats.totalTime, phaseTimes: stats.phaseTimes, classpathCount: stats.classpathCount, libraryCount: stats.libraryCount, isLegacy: isLegacyVersion(manifest) });
